...
>>>>>>> 3b8d57d1f778c3d2e1a14f4f658cdae338f54366
...
void rencontre(struct position * robot, int n, double * tableau ) {
  ...
  for ( int i=0 ; i<n-1; i++){
    ...
    for ( int j=i+1 ; j<n; j++){
        ...
        //printf("il y a eu une rencontre\n\n ");
        printf(" le robot %d avec le robot numero %d , x= %d" ,j,i,robot[i].x);
        printf(" y= %d" ,robot[i].y);
...
int main(int argc, char * argv[]) {
  ...
  input("coordonnée x du solute (entre [0:100]) : x=",&a);
  int x0 = strtol(a.phrase, NULL, 10);
  
  
  input("coordonnée y du solute (entre [0:100]) : y=",&a);
  int y0 = strtol(a.phrase, NULL, 10);
  
  // verifier que x0 et y0 sont bien des int
  
  if ((x0 | y0)>100 | (x0 | y0)<0 | n<=0 | n>=100) { return printf( "les coordonnées  du solute doivent être comprise en tre [0:100] , le nb de batéries doit être compris entre [0;100]");}
